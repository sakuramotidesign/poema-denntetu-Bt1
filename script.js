...
function isHiddenName(name) {
  return HIDDEN_NAMES.includes(name);
}
...
function resetAllData() {
  ...
  ["A", "B"].forEach(k => {
    teams[k].position = 0;
    teams[k].phase = "SELECT_RELIEF";
    teams[k].waitUntil = null;
    teams[k].nextPosition = null;
    teams[k].history = [];
    teams[k].dice = null;
    teams[k].plus1Ready = false;
    teams[k].relief = null;
    teams[k].reliefCount = { plus3: 0, double: 0 };
    teams[k].reliefUsedThisRoll = false;
  });
  ...
  document.getElementById("diceResultA").textContent = "-";
  document.getElementById("diceResultB").textContent = "-";
...
function loadChat() {
  ...
  database.ref("chat").limitToLast(50).on("child_added", snap => {
    ...
    database.ref("players/" + msg.playerId).once("value", pSnap => {
      ...
      div.textContent = `[${p.team}] ${p.displayName}：${msg.text}`;
      log.appendChild(div);
      log.scrollTop = log.scrollHeight;
    });
  });
